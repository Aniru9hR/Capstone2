...
st.title(":orange[StarWars Characters!!]")
rows = run_query("SELECT name,height,mass,gender,hair_color,skin_color,homeworld,films,starships from character;")
character_df = pd.DataFrame(rows, columns=['Name', 'Height', 'Mass',
                               'Gender', 'Hair color',
...
# Establish a filepath to the character_data.csv file
filepath = os.path.join(Path(__file__).parents[1], 'data/character_data.csv')
